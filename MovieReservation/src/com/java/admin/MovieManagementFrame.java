...
public class MovieManagementFrame extends JFrame {
	...
	public MovieManagementFrame(String id) {
		...
		for (int i = 0; i < movies.size(); i++) {
			...
			row.addElement(movies.get(i).getM_count());
		...
		movieDeleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Object m_name = movieTable.getValueAt(row, 0);
				
				tableModel.removeRow(row);
				
				Movie movie = new Movie();
				movie.setM_name((String)m_name);
				
				MovieDAO dao = MovieDAO.getInstance();
				int result = dao.delete(movie);
				
				if (1 == result) {
					JOptionPane.showMessageDialog(null, "영화삭제 완료");
					dispose();
				} else {
					JOptionPane.showMessageDialog(null, "영화삭제 실패");
					dispose();
				}
			}
